...
// TestArtifactTamper_AppendInjection: re-submission appends attacker content to
// the stored task (both texts present) => high/ConfirmedExploit artifact injection.
func TestArtifactTamper_AppendInjection(t *testing.T) {
	...
	if len(findings) == 0 {
		t.Fatal("expected a finding for append injection")
	}
